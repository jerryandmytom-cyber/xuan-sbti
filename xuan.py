...
def ziwei_cmd(msg):
    # 紫微斗数是付费服务，直接弹付费模板
    send_paid_template(msg.chat.id, 'ziwei')
...
def handle(msg):
    ...
    # ── 流年大运 / 合婚分析（付费服务）─────────────────
    # 这些关键词在 get_paid_key 中已覆盖，但为确保
    # bazi_context/yinyuan_context 内也能触发，在此提前拦截
    if text in ["流年大运", "流年", "大运"]:
        send_paid_template(msg.chat.id, 'liunian')
        return
...
# ============ 玄学行业&用户痛点整改方案（内部全功能版）============
XUAN_INTERNAL_RECTIFICATION_PLAN = {
    "行业痛点整改": [
        "建立“结论可追溯”机制：每条判断都附带触发规则（五行、十神、冲合刑害）与置信度区间。",
        "建立统一服务SOP：采集信息→结构化排盘→风险分级→建议输出→复盘回访，减少大师口径差异。",
        "新增合规词典：屏蔽绝对化承诺（如“100%转运”），统一改为概率和条件表达，降低投诉风险。",
        "增加风险提示分层：健康、财务、情感类内容必须附“理性决策提示+专业机构建议”。",
        "建立质量监控看板：按满意度、复购率、投诉率、退款率、响应时长进行周维度追踪。"
    ],
    "用户痛点整改": [
        "解决“看不懂”：输出结构改为“结论先行 + 原因解释 + 可执行建议 + 时间窗口”。",
        "解决“建议空泛”：每项建议增加执行动作（例如颜色、方位、沟通策略、时间安排）与周期。",
        "解决“等待久”：加入极速首答模板（30秒内），深度报告异步补全并推送。",
        "解决“前后不一致”：同一用户建立命理档案ID，二次咨询自动关联历史结论。",
        "解决“付费不透明”：服务价目、交付清单、交付时效、售后方式前置展示。"
    ],
    "内部执行里程碑": [
        "T+3天：上线结构化输出模板和合规词典。",
        "T+7天：上线置信度标注与投诉关键词预警。",
        "T+14天：完成全链路SOP培训并启用周报复盘机制。"
    ]
}


def generate_internal_rectification_text() -> str:
    """内部版：输出完整整改方案，用于运营、质检、培训。"""
    sections = ["🧭 玄学服务新整改方案（内部全功能版）"]
    sections.append("\n【一、行业痛点整改】")
    sections.extend([f"{i+1}. {x}" for i, x in enumerate(XUAN_INTERNAL_RECTIFICATION_PLAN["行业痛点整改"])])
    sections.append("\n【二、用户痛点整改】")
    sections.extend([f"{i+1}. {x}" for i, x in enumerate(XUAN_INTERNAL_RECTIFICATION_PLAN["用户痛点整改"])])
    sections.append("\n【三、执行里程碑】")
    sections.extend([f"{i+1}. {x}" for i, x in enumerate(XUAN_INTERNAL_RECTIFICATION_PLAN["内部执行里程碑"])])
    return "\n".join(sections)
